...
// Общее число конкурсов для этой организации
$countStmt  = $conn->prepare("SELECT COUNT(*) AS cnt FROM contests WHERE organization_name = ?");
$countStmt->bind_param('s', $org);
$countStmt->execute();
$total      = $countStmt->get_result()->fetch_assoc()['cnt'];
$totalPages = (int)ceil($total / $limit);
$countStmt->close();

// Получаем страницу конкурсов
$stmt = $conn->prepare(
    "SELECT contest_id, name, order_number, level, start_date, end_date, age_category, description
     FROM contests
     WHERE organization_name = ?
     ORDER BY start_date DESC
     LIMIT ?, ?"
);
$stmt->bind_param('sii', $org, $offset, $limit);
$stmt->execute();
...
<head>
  ...
  <style>
    ...
    .content-container { background:rgba(255,255,255,0.95); padding:30px; border-radius:20px; width:100%; max-width:1200px; min-height:1000px; box-shadow:0 12px 40px rgba(0,0,0,0.2); display:flex; flex-direction:column; }
    .page-header { display:flex; justify-content:space-between; align-items:center; margin-bottom:20px; }
    .btn { background:linear-gradient(135deg,#8A2BE2,#4B0082); color:#fff; padding:12px 24px; border-radius:999px; text-decoration:none; font-weight:500; font-size:16px; display:inline-block; transition:background .3s,transform .2s; }
    .btn:hover { background:linear-gradient(135deg,#9575CD,#B39DDB); transform:scale(1.05); }
    .search-bar { display:flex; gap:10px; margin-bottom:16px; flex-wrap:nowrap; }
    .search-bar select { flex:0 0 auto; }
    .search-wrapper { position:relative; flex:1 1 auto; }
    .search-wrapper input { width:100%; padding:10px; border:1px solid #ccc; border-radius:6px; font-size:16px; box-sizing:border-box; }
    .search-suggestions { position:absolute; top:100%; left:0; background:#fff; border:1px solid #ccc; width:100%; max-height:200px; overflow-y:auto; display:none; z-index:10; }
    ...
    .actions a.btn { margin:0; padding:8px 16px; font-size:14px; }
...
<body>
  <div class="content-container">
    <div class="page-header">
      ...
      <a href="add_contest_user.php" class="btn">➕ Добавить</a>
    </div>
    ...
    <table>
      ...
      <tbody id="contestsTable">
        ...
        <tr data-id="<?= $row['contest_id'] ?>">
          ...
          <td class="actions">
            <a href="participants.php?contest_id=<?= $row['contest_id'] ?>" class="btn" onclick="event.stopPropagation()">Участники</a>
          </td>
        </tr>
        <?php endwhile; ?>
      </tbody>
  ...
  <script>
    ...
    columnSelect.addEventListener('change', () => { searchInput.value = ''; filterTable(); });

    rows.forEach(row => row.addEventListener('click', () => {
      window.location.href = `edit_contest_user.php?id=${row.getAttribute('data-id')}`;
    }));
  </script>
</body>
</html>