...
if (!isset($_GET['id'], $_GET['event_id']) || !is_numeric($_GET['id']) || !is_numeric($_GET['event_id'])) {
    header("Location: event_participants.php");
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete'])) {
    ...
    if ($delStmt->execute()) {
        header("Location: event_participants.php?event_id={$event_id}");
        exit;
    } else {
...
$stmt = $conn->prepare("
    SELECT ep.organization_name, ep.class, ep.association, ep.responsible_id,
...
<head>
  <meta charset="UTF-8">
  <title>Редактировать участника мероприятия</title>
  <style>
    body {
...
<body>
  <div class="form-container">
    ...
    <form method="POST">
      <input type="text" value="<?= htmlspecialchars($row['organization_name'], ENT_QUOTES) ?>" readonly>
      <input type="text" value="<?= htmlspecialchars($row['child_name'], ENT_QUOTES) ?>" readonly>
    ...
    <a href="event_participants.php?event_id=<?= $event_id ?>" class="back-link">← Назад к списку участников</a>
  </div>
</body>
</html>
