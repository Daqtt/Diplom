    ...
    if ($search !== '') {
        ...
        switch ($column) {
            case 'child':
                ...
                break;
            case 'class':
                $where   .= ' AND p.class LIKE ?';
                $types   .= 's';
                $params[] = $param;
                break;
            case 'assoc':
                $where   .= ' AND p.association LIKE ?';
                $types   .= 's';
                $params[] = $param;
                break;
            case 'leader':
...
<head>
  ...
  <style>
    ...
    /* убрали организацию */
    th.col-child,  td.col-child  { width: 25%; }
...
<body>
  <div class="container">
    <div class="header">
      <a href="summer_sessions_user.php" class="btn">← Смены</a>
      <h1>Участники смены</h1>
      <!-- Кнопка "Добавить" убрана -->
      <div style="width: 96px;"></div> <!-- сохраняем отступ -->
    </div>
  ...
  <script>
    ...
    function loadParticipants() {
      ...
      fetch(`session_participants_user.php?ajax=1&session_id=${sessionId}&search=${search}&column=${column}&page=${page}`)
        .then(res => res.json())
        .then(j => {
          ...
          j.data.forEach(r => {
            ...
            tr.innerHTML = `
              <td class="col-child">${r.child_name}</td>
              <td class="col-class">${r.class}</td>
              <td class="col-assoc">${r.association}</td>
              <td class="col-leader">${r.leader_name}</td>
              <td class="col-result">${r.result}</td>
            `;
